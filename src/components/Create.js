...
const Create = () => {
  const [formData, setFormData] = useState({
    ...
    rerouting_done: "",
    additional_remarks: "",
    survey_date: new Date().toISOString().split("T")[0], // Set current date as survey date
  });

  useEffect(() => {
    fetchMaxId(); // Fetch the maximum ID when the component mounts
  }, []);
  ...
  const fetchMaxId = async () => {
    try {
      ...
      setFormData((prevData) => ({ ...prevData, id: maxId + 1 })); // Set ID to maxId + 1
    } catch (error) {
  ...
  const fetchExistingEntries = async (latitude, longitude) => {
    try {
      const response = await axios.get(
        "https://onmapi.multinet.com.pk:81/customers"
      );
      const data = response.data;
      return data.filter(
        (entry) => entry.latitude === latitude && entry.longitude === longitude
      );
    } catch (error) {
